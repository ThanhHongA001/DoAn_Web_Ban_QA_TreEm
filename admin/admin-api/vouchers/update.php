...
try {
    ...
    $stmt = $pdo->prepare("UPDATE vouchers SET description = ?, discount_percent = ?, quantity = ?, expiry_date = ? WHERE code = ?");
    $stmt->execute([$description, $discount_percent, $quantity, $expiry_date, $code]);
